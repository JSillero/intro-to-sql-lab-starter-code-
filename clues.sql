...
-- Write SQL query here
SELECT cities.* FROM countries JOIN cities ON countries.code = cities.countrycode 
WHERE countries.code = 'SMR' and cities.name != countries.name; 

-- Clue #5: Oh no, she pulled a switch – there are two cities with very similar names, but in totally different parts of the globe! She's headed to South America as we speak; go find a city whose name is like the one we were headed to, but doesn't end the same. Find out the city, and do another search for what country it's in. Hurry!
--Serravalle
-- Write SQL query here
SELECT cities.name as city, countries.name as country   FROM countries JOIN cities ON countries.code = cities.countrycode 
...
-- Write SQL query here
SELECT cities.name as city FROM countries JOIN cities ON countries.capital = cities.id 
